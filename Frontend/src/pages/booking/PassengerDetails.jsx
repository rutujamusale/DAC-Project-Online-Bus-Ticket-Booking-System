...
const PassengerDetails = ({
  ...
  useEffect(() => {
    ...
    const hasReservedSeats = selectedSeats.some(
      (seat) => seat.status === "RESERVED"
    );
    if (hasReservedSeats) {
      setSeatsReserved(true);
      toast.info(
        "Your seats are already reserved. You can continue with payment."
      );
    }
    ...
    if (!dataRestored) {
      const savedData = localStorage.getItem(
        `passengerForm_${scheduleInfo.id}_${selectedSeats
          .map((s) => s.id)
          .join("_")}`
      );
    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          if (!showPayment) {
            toast.error(
              "Time's up! Your seats have been released. Please go back and select seats again."
            );
            // Clear saved form data before going back
            if (scheduleInfo && selectedSeats) {
              const storageKey = `passengerForm_${
                scheduleInfo.id
              }_${selectedSeats.map((s) => s.id).join("_")}`;
  ...
  useEffect(() => {
    return () => {
      ...
      if (!showPayment && scheduleInfo && selectedSeats) {
        const storageKey = `passengerForm_${scheduleInfo.id}_${selectedSeats
          .map((s) => s.id)
          .join("_")}`;
        localStorage.removeItem(storageKey);
      }
  ...
  const handleInputChange = (index, field, value) => {
    ...
    if (field === "phone") {
      ...
      if (cleanedValue.startsWith("+91")) {
        ...
        const digitsAfterPrefix = cleanedValue
          .substring(3)
          .replace(/\D/g, "")
          .slice(0, 10);
  ...
  const validatePassengerData = (passenger, index) => {
    ...
    if (!validatePhoneNumber(cleanPhone)) {
      toast.error(`Please enter a valid Indian mobile number for Passenger ${
        index + 1
      }. 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const lockResponse = await axios.post(
        "http://localhost:8080/api/seats/lock",
        {
      ...
      setSeatsReserved(true);
      toast.success(
        "Seats reserved successfully! You have 10 minutes to complete payment."
      );
      ...
      if (bookingResponse.data) {
        toast.success(
          "Booking created successfully! Please complete payment to confirm your seats."
        );
      ...
      if (
        ...
        error.response.data.message.includes("Invalid Indian contact number")
      ) {
        toast.error(
          "❌ Invalid phone number format! Please enter a valid Indian mobile number starting with 6, 7, 8, or 9 (e.g., 9876543210)"
        );
        ...
        if (errorMessage.includes("Validation failed")) {
          toast.error(
            "❌ Please check your input data. Make sure all fields are filled correctly."
          );
        } else {
  ...
  const handlePaymentComplete = (paymentResponse) => {
    ...
    toast.success(
      "Payment completed successfully! Your seats are now confirmed."
    );
  ...
  const handleBackToBooking = () => {
    ...
    toast.info(
      "Returned to booking details. You can continue with payment or modify details."
    );
  };
            ...
            <div className="card-body">
              ...
              <div className="alert alert-info mb-4">
                ...
                <p className="mb-1">
                  ...
                  <strong>Time:</strong> {scheduleInfo?.departureTime}
                </p>
                <p className="mb-0">
              ...
              {seatsReserved && (
                <div
                  className={`alert ${
                    timeLeft < 300 ? "alert-danger" : "alert-warning"
                  } mb-4`}
                >
                  <h6>
                    <i className="fas fa-clock me-2"></i>Payment Time Limit
                  </h6>
                  <div className="mb-2">
                    ...
                    <div className="progress" style={{ height: "8px" }}>
                      <div
                        className={`progress-bar ${
                          timeLeft < 300 ? "bg-danger" : "bg-warning"
                        }`}
                  ...
                  <p className="mb-1">
                    You have <strong>10 minutes</strong> to complete payment. If
                    you don't pay within this time, your seats will be released.
                  </p>
                  <p className="mb-0">
              ...
              {!seatsReserved && (
                <div className="alert alert-info mb-4">
                  <h6>
                    <i className="fas fa-info-circle me-2"></i>Important
                    Information
                  </h6>
                  <p className="mb-1">
                    Your seats are currently selected but not reserved.
                  </p>
                  <p className="mb-1">
                    When you click "Confirm Booking", your seats will be
                    reserved for <strong>10 minutes</strong>.
                  </p>
                  <p className="mb-0">
                    You must complete payment within 10 minutes to confirm your
                    booking.
                  </p>
                </div>
              )}
                      ...
                      <div className="row">
                        <div className="col-md-6 mb-3">
                          ...
                          <input
                            ...
                            className={`form-control ${
                              passenger.phone &&
                              !validatePhoneNumber(passenger.phone)
                                ? "is-invalid"
                                : passenger.phone &&
                                  validatePhoneNumber(passenger.phone)
                                ? "is-valid"
                          ...
                          <small className="text-muted">
                            Enter 10-digit Indian mobile number starting with 6,
                            7, 8, or 9
                          </small>
                          {passenger.phone &&
                            !validatePhoneNumber(passenger.phone) && (
